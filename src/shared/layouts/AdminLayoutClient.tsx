...
function getIcon(iconName: string): LucideIcon {
    ...
    // Mapeo manual para los íconos más comunes o fallback
...
export default function AdminLayoutClient({
    ...
    if (user.rol !== ROLES.ADMIN && user.rol !== ROLES.DUENO) {
        return (
            <div className="flex items-center justify-center min-h-screen bg-slate-50">
                <div className="text-center bg-white p-12 rounded-[3rem] shadow-2xl border border-slate-100 max-w-md mx-4 animate-in zoom-in-95 duration-500">
                    ...
                    <p className="text-slate-500 font-medium leading-relaxed">
                        Esta zona está reservada para administradores. Si crees que esto es un error, contacta al soporte técnico.
                    </p>
                    <button
    ...
    return (
        <AppShell title="Administración" user={user} links={links}>
            {children}
        </AppShell>
    );
}